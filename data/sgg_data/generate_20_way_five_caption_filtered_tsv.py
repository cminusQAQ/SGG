...
data_split = '20_way_caption_five_filtered'
data_tag = 'parser-ours_synonym_match'
train_file_dict = {
    'opt-new': 'opt_5cap_hard_match_20_fix_noun_lemma.json',
    'opt': 'opt_5cap_hard_match_20.json',
    'opt-pevl': 'pevl_vg_5caption_20.json',
    'parser+opt': 'combine_parser_opt_5cap_hard_match_20.json',
    'parser-fix-mutlilabel': 'parser_5cap_dense_fix_hard_match_bug_20.json',
    'parser+opt-fix-multilabel': '5cap_combine_parser_opt_fix_hard_match_bug_20.json',
    'opt-fix-multilabel': 'opt_5cap_fix_hard_match_bug_20.json',
    'opt-iccv2021_match': 'opt_5cap_synonyms_iccv2021_20.json',
    'parser-iccv2021_match': 'parser_5cap_synonyms_iccv2021_20.json',
    'opt-ours_synonym_match': 'opt_5cap_my_synonyms_20.json',
    'parser-ours_synonym_match': 'parser_5cap_my_synonyms_20.json'
}
...
def collect_train_pos_samples(train_data, train_info, meta_mapping):
    ...
    for item in train_info:
        key = item['image_path']
        img_key_to_info[key] = item

    value_lines = []
    # {image_path: rel_pairs}
    image_label_pairs = {}
    item_id = 0
    for img_item_id, train_item in enumerate(tqdm.tqdm(train_data)):
        sample_id = train_item['id']
        img_key = train_item['image_path']

        img_info = img_key_to_info[img_key]
        assert img_info['image_path'] == img_key
        ...
        img_idx = img2idx[img_key]
        w_resize_ratio, h_resize_ratio = get_w_h_ratio(img_idx)

        triplets, pairs = train_item['triplets'], train_item['box_pair']
        for triplet_text, box_pair in zip(triplets, pairs):
            sub,  predicate_name, obj = triplet_text.split('-')
            sub_coord, obj_coord = box_pair
            ...
            if img_key not in image_label_pairs:
                image_label_pairs[img_key] = []
            image_label_pairs[img_key].append((sub_idx, obj_idx))
            ...
            values = [str(item_id), str(sample_id), instruction,
...
def collect_train_NA_samples(train_info, img2rel_pairs, meta_mapping):
    ...
    for item_id, train_img_item in enumerate(tqdm.tqdm(train_info)):
        img_key = train_img_item['image_path']
        if img_key not in img2rel_pairs:
            skip_img += 1
            continue
        ...
        for sub_idx, (sub_name, sub_box) in enumerate(zip(obj_label_names, boxes)):
            for obj_idx, (obj_name, obj_box) in enumerate(zip(obj_label_names, boxes)):
                if obj_idx == sub_idx:
                    continue
                if (sub_idx, obj_idx) in img2rel_pairs[img_key]:
                    skip_cnt += 1
                    continue
...
# %%
train_rel_data = json.load((split_dir / f'{train_file_dict[data_tag]}').open())
train_info = json.load((split_dir / 'caption_all_info.json').open())
...
for epoch in tqdm.tqdm(range(epoch)):
    ...
    with (split_dir / f'{data_prefix}{data_tag}_train_NA{ratio}_E{epoch}.tsv').open('w') as out_file:
