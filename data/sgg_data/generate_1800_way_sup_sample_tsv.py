...
vg_dict = json.load((split_dir / 'VG-dicts.json').open())

ans2label = vg_dict['predicate_to_idx']
ans2label['no relation'] = 0
pickle.dump(ans2label, (split_dir / '1800_way_ans2label.pkl').open('wb'))
...
# %%
train_rel_data = json.load((split_dir / 'sup_train_1800.json').open())
train_info = json.load((split_dir / 'sup_info_1800.json').open())
...
def collect_test_samples(test_info, meta_mapping, name, img_cnt=200):
    value_lines = []
    image_st_ed = {}
    image_label = {}
    line_idx = 0
    ...
    if img_cnt == 0:
        img_cnt = len(test_info)

    for item_id, test_item in enumerate(tqdm.tqdm(test_info[:img_cnt])):
        val_id = test_item['id']
        img_idx = img2idx[test_item['image_path']]
        w_resize_ratio, h_resize_ratio = get_w_h_ratio(img_idx)

        boxes = test_item['bboxes']
        triplets = set(map(tuple, test_item['gt_triple']))
        pair2label = {}
        for s, o, p in triplets:
            if (s, o) not in pair2label:
                pair2label[(s, o)] = []
            pair2label[(s, o)].append(p)
            p_name = meta_mapping['idx_to_predicate'][str(p)]
            rel_label_cnt[p_name] = rel_label_cnt.get(p_name, 0) + 1

        obj_label_ids = test_item['labels']
        obj_label_names = [meta_mapping['idx_to_label'][str(i)] for i in obj_label_ids]
        obj_label_name_str = '&&'.join(obj_label_names)
        assert len(obj_label_names) == len(boxes)
        ...
        for sub_idx, (sub_name, sub_box) in enumerate(zip(obj_label_names, boxes)):
            for obj_idx, (obj_name, obj_box) in enumerate(zip(obj_label_names, boxes)):
                ...
                if (sub_idx, obj_idx) not in pair2label:
                    answer = f'1.0|!+no relation'
                else:
                    answer = f'1.0|!+{meta_mapping["idx_to_predicate"][str(pair2label[(sub_idx, obj_idx)][0])]}'

                values = [str(line_idx), str(item_id), instruction,
                          answer, obj_label_name_str, str(img_idx)]
                value_line = '\t'.join(values) + '\n'
                value_lines.append(value_line)
                line_idx += 1
        image_st_ed[val_id] = (st, line_idx)
        image_label[val_id] = {'boxes': boxes, 'labels': obj_label_ids,
                               'gt_triple': test_item['gt_triple']}
    with (split_dir / f'{data_prefix}{name}_{img_cnt}.tsv').open('w') as out_file:
        out_file.writelines(value_lines)

    json.dump(image_st_ed, (split_dir / f'{data_prefix}{name}_sample_st_ed_{img_cnt}.json').open('w'))
    json.dump(image_label, (split_dir/ f'{data_prefix}{name}_label_{img_cnt}.json').open('w'))
    return rel_label_cnt

# %%
test_data = json.load((split_dir / 'test_data_dedup1800.json').open())
test_p_counter = collect_test_samples(test_data, vg_dict, 'test', 0)

# %%
val_data = json.load((split_dir / 'val_data_dedup1800.json').open())
val_p_counter = collect_test_samples(val_data, vg_dict, 'val', 2000)
...
# %%
val_data = json.load((split_dir / 'val_data_dedup1800.json').open())
val_p_counter = collect_test_samples(val_data, vg_dict, 'val', 1000)
