...
def device_of(m): return next(m.parameters()).device
...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.distill_alpha = cfg.common.distill_alpha
        if self.distill == 'default':
            self._model_m = copy.deepcopy(self._model)
            self._model_m.requires_grad_(False)
            origin_dict = self._model.state_dict()
            copy_dict = self._model_m.state_dict()
            for k in origin_dict:
                assert origin_dict[k] is not copy_dict[k]
                assert torch.equal(origin_dict[k], copy_dict[k])
                if k not in copy_dict:
                    print(f'Missing {k}')
                    exit()
            self.momentum = 0.95
            # self._model_m.to(self.device)
            print(f'Done {self.momentum} {self.device} {device_of(self._model)}, {device_of(self._model_m)}')

    def _momentum_update(self):
        if device_of(self._model) != device_of(self._model_m):
            print(f'From {device_of(self._model_m)} to {device_of(self._model)}')
            self._model_m.to(device_of(self._model))
        for param, param_m in zip(self._model.parameters(), self._model_m.parameters()):
            # print(param.device, param_m.device)
    ...
    def model_m(self):
        if self._wrapped_model is None:
            if self.use_distributed_wrapper:
                ...
                self._wrapped_model = self._model_m
        return self._wrapped_model
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                if self.cfg.checkpoint.use_ema_weights_to_init_param and "extra_state" in state and "ema" in state[
                    "extra_state"]:
                    logger.info(
                        "use_ema_weights_to_init_param = True, will use EMA weights in the ckpt to init the model param...")
                    ema_state_dict = state["extra_state"]["ema_fp32_params"] if "ema_fp32_params" in state[
                        "extra_state"] else state["extra_state"]["ema"]
                    self.model.load_state_dict(
                        ema_state_dict, strict=True, model_cfg=self.cfg.model
                    )
                    if self.distill == 'default':
                        logger.info('Load Model_m together with Model')
                        self.model_m.load_state_dict(
                            ema_state_dict, strict=True, model_cfg=self.cfg.model
                        )
                else:
                    self.model.load_state_dict(
                    ...
                    if self.distill == 'default':
                        logger.info('Load Model_m together with Model 2')
                        self.model_m.load_state_dict(
                            state["model"], strict=True, model_cfg=self.cfg.model
                        )
                # save memory for later steps
                if not (self.cfg.ema.store_ema and (self.cfg.checkpoint.use_latest_weights_to_init_ema or not (
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            # if distributed_utils.get_data_parallel_rank() in [0, 1]:
            #     print(f'Rank-{distributed_utils.get_data_parallel_rank()}, {sample["id"]}')

            # sample_id_list = self._all_gather_list_sync([torch.tensor([int(x) for x in sample['id']])])[0]
            # if distributed_utils.get_data_parallel_rank() == 0:
            #     logger.info(f'Samples in batch: {sample_id_list}')
            ...
            try:
                with maybe_no_sync():
                    ...
                    data_source = sample['id'][0]
                    # print(sample['id'])
                    sample['id'].fill('0')
                    # print('After', sample['id'])
                    if self.distill == 'default' and data_source == 'visualDS':
                        with torch.no_grad():
                            self._momentum_update()
                            logging_output, logits, sample_ids, time_info = self.valid_step(sample, do_distill=True)
                            n_sample = len(sample['conf'])
                            old_conf = sample["conf"].tolist()
                            for item_idx in range(n_sample):
                                KB_label = sample['KB_label'][item_idx]
                                annotation_label_idx = sample['annotation_label_idx'][item_idx]
                                if annotation_label_idx == 0:
                                    continue
                                selected_logits = [logits[item_idx, rel_idx] for rel_idx in KB_label]
                                selected_scores = torch.tensor(selected_logits).softmax(-1)
                                annotation_label_score = selected_scores[KB_label.index(annotation_label_idx)]
                                sample['conf'][item_idx] = self.distill_alpha * annotation_label_score + (1 - self.distill_alpha) / len(KB_label)
                            new_conf = sample["conf"].tolist()
                            conf_diff = [f'{a - b:.2f}' for a, b in zip(new_conf, old_conf)]
                            # logger.info(f'{sample["id"].tolist()} {sample["answer"]} diff: {conf_diff} ')
    ...
    def valid_step(self, sample, raise_oom=False, do_distill=False):
        if do_distill:
            logger.info(f'Distill with momentum {self.momentum} alpha {self.distill_alpha}')
            model = self.model_m
        else:
            model = self.model
        """Do forward pass in evaluation mode."""
        time_cost_info = {}
        prepare_model_time_st = time.time()
        if self.tpu:
            import torch_xla.core.xla_model as xm
        ...
        with torch.no_grad():
            model.eval()
            self.criterion.eval()
            time_cost_info['prepare_model'] = time.time() - prepare_model_time_st

            prepare_data_time_st = time.time()
            sample, is_dummy_batch = self._prepare_sample(sample)
            time_cost_info['prepare_sample'] = time.time() - prepare_data_time_st

            try:
                task_valid_time_st = time.time()
                ...
                time_cost_info['task_valid'] = time.time() - task_valid_time_st
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
                    if not raise_oom:
        ...
        # gather logging outputs from all replicas
        aggregate_output_time_st = time.time()
        sgg_metric_info = None
        if self.data_parallel_world_size > 1:
            ...
            cal_sgg_metric = self._all_gather_list_sync([cal_sgg_metric])[0]
            logits_list = [x['logits'] for x in cal_sgg_metric]
            sample_id_list = [x['sample_ids'] for x in cal_sgg_metric]
            # if distributed_utils.get_data_parallel_rank() == 0:
            #     logger.info(f'Samples in batch: {sample_id_list}')

            logits = torch.cat(logits_list)
            sample_ids = torch.cat(sample_id_list)

            # if self.data_parallel_rank <= 0:
            #     logger.info(f'validate sample_ids: {sample_ids.shape} {sample_ids}, logits:{logits.shape} {logits}')
        time_cost_info['collect_output'] = time.time() - aggregate_output_time_st
        ...
        return logging_output, logits, sample_ids, {**time_cost_info, **vs_time_cost_info}
